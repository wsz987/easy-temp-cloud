...
func TestUppyCoreModuleIsServedAsJavaScript(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/assets/vendor/uppy/core.js", nil)
	response := httptest.NewRecorder()
	newRouter(service).ServeHTTP(response, request)
	if got := response.Header().Get("Content-Type"); got != "text/javascript; charset=utf-8" && got != "application/javascript" {
